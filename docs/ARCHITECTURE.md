...
純粋ロジックのテストにはNode.js標準のテストランナーを使います。描画に依存しないロジックは `src/game/` に置き、追加のテストフレームワークなしで検証します。
...
`main` ブランチへのpushを契機に、テスト、TypeScript型チェック、Viteビルドを順に行い、成功した `dist` のみをPagesへデプロイします。
